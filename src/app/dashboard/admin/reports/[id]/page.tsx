...
export default async function AdminReportDetailPage({ params }: PageProps) {
  ...
  const report = await prisma.report.findUnique({
    ...
    include: {
      ...
      targetUser: {
        select: { username: true, email: true, status: true, role: true },
      },
      reviewerUser: { select: { username: true } },
          ...
          >
            <div>
              ...
              <h1 style={{ margin: 0, fontSize: "42px", lineHeight: 1 }}>
                {getReportLabel(report)} #{report.id.slice(0, 8)}
              </h1>
            </div>

            <div style={{ display: "flex", gap: "12px", flexWrap: "wrap" }}>
              <Link href="/dashboard/admin/reports" style={topLinkStyle}>
                Voltar
              </Link>
        ...
        <section style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "18px" }}>
          <div style={panelStyle}>
            ...
            <div style={{ display: "grid", gap: "12px", marginTop: "16px" }}>
              ...
              <InfoRow
                label="Denunciante"
                value={`@${report.createdByUser.username} • ${report.createdByUser.email}`}
              />
              <InfoRow
                label="Alvo"
                value={
              ...
              <InfoRow
                label="Criada em"
                value={new Date(report.createdAt).toLocaleString("pt-BR")}
              />
              <InfoRow
                label="Review feita por"
                value={report.reviewerUser ? `@${report.reviewerUser.username}` : "-"}
              />
          ...
          <div style={panelStyle}>
            ...
            <form action={resolveReport} style={{ display: "grid", gap: "12px", marginTop: "14px" }}>
              ...
              <textarea
                name="adminNote"
                rows={4}
                placeholder="Nota administrativa ao resolver"
                style={inputStyle}
              />
            ...
            <form action={rejectReport} style={{ display: "grid", gap: "12px", marginTop: "14px" }}>
              ...
              <textarea
                name="adminNote"
                rows={4}
                placeholder="Nota administrativa ao rejeitar"
                style={inputStyle}
              />
